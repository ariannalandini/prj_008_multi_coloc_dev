...
option_list <- list(
  ...
  make_option("--tabix_bin", type="character", default="/ssu/gassu/software/htslib-tools/1.14/tabix", 
              help="Path to tabix software", metavar="character"),
...
# Check that at least one genome-wide significant SNP across all traits was left (could have been lost in munging during alignment)
    check_ind.snps <- unlist(conditional.datasets, recursive = FALSE) %>% .[grep("ind.snps", names(.))]
    if(!any(sapply(check_ind.snps, function(x){any(x$p < 5e-8 | x$pJ < 5e-8)}))){
